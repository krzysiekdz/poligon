...
begin
  ...
  try
    BinDB.ExecSQL('DELETE FROM bs_blogtags_pr WHERE idblog='+IntToStr(_ID));
    TBinFMUtils.ExplodeStr1(L1,GetValue('wtags',''),';',FALSE);
    for i:=0 to L1.Count-1 do
    begin
      S:=lowercase(L1[i]);
      k:=BinDB.GetValueI('SELECT id FROM bs_blogtags WHERE pname='''+BinDB.AddSlashes(S)+'''',0);
      if k<=0 then
      begin
         ...
         E:=BinDB.InsertUpdateRow('bs_blogtags');
      ...
      E:=BinDB.InsertUpdateRow('bs_blogtags_pr');
      E.AddL('idblog','int',_ID);
      E.AddL('idtag','int',K);
      E.Execute(0);
      E.Free;
    end;
