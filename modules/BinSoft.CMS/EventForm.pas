...
begin
  ...
  try
    BinDB.ExecSQL('DELETE FROM bs_eventtags_pr WHERE idevent='+IntToStr(_ID));
    TBinFMUtils.ExplodeStr1(L1,GetValue('wtags',''),';',FALSE);
    for i:=0 to L1.Count-1 do
    begin
      S:=lowercase(L1[i]);
      k:=BinDB.GetValueI('SELECT id FROM bs_eventtags WHERE pname='''+BinDB.AddSlashes(S)+'''',0);
      if k<=0 then
      begin
         ...
         E:=BinDB.InsertUpdateRow('bs_eventtags');
      ...
      E:=BinDB.InsertUpdateRow('bs_eventtags_pr');
      E.AddL('idevent','int',_ID);
      E.AddL('idtag','int',K);
      E.Execute(0);
      E.Free;
    end;
